...
const { getDateOnly, getMonthYear, getMonthRangeIST } = require('../../utils/time');
...
const getManagerDashboard = async (managerUserId, companyId) => {
  ...
  const records = await AttendanceRecord.find(
    { companyId, userId: { $in: teamUserIds }, date: attendanceRepository.istDayFilter(today) },
    null,
    { companyId }
  )
  ...
  const team = await Promise.all(
    teamProfiles.map(async (profile) => {
      ...
      return {
        ...
        hasOpenSession,
        punchIn: record ? workingHoursEngine.getFirstPunchIn(record) || null : null,
        punchOut: record
          ? hasOpenSession
            ? null
...
const getHrDashboard = async (companyId) => {
  const today = getDateOnly();
  const records = await AttendanceRecord.find(
    { companyId, date: attendanceRepository.istDayFilter(today) },
    null,
    { companyId }
  )
    .populate('userId', 'firstName lastName fullName email')
