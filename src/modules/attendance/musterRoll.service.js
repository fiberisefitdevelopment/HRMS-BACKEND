...
const toDateKey = (date) => formatDateOnly(date);
...
const buildDayMeta = (year, month, holidayMap) => {
  // Days in month via UTC noon to avoid local DST edge cases
  const daysInMonth = new Date(Date.UTC(year, month, 0)).getUTCDate();
  const days = [];

  for (let day = 1; day <= daysInMonth; day += 1) {
    // Noon UTC on that calendar day → stable IST weekday for India
    const date = getDateOnly(new Date(Date.UTC(year, month - 1, day, 6, 0, 0)));
...
const formatPunchSummary = (record) => {
  if (!record) {
    return {
      attendanceId: null,
      punchIn: null,
  ...
  const punchOut = workingHoursEngine.hasOpenSession(record)
    ? null
    : workingHoursEngine.getLastPunchOut(record);

  return {
    attendanceId: record._id || null,
    punchIn: punchIn?.timestamp ? formatDateTimeIST(punchIn.timestamp) : null,
    punchInDisplay: punchIn?.timestamp ? formatTimeIST(punchIn.timestamp) : null,
    punchOut: punchOut?.timestamp ? formatDateTimeIST(punchOut.timestamp) : null,
    punchOutDisplay: punchOut?.timestamp ? formatTimeIST(punchOut.timestamp) : null,
...
const getMusterRoll = async (companyId, query) => {
  ...
  const monthEnd = require('../../utils/time').addDays(monthEndExclusive, -1);
    ...
    CompOffRequest.find(
      {
        companyId,
        employeeProfileId: { $in: profileIds },
        attendanceDate: { $gte: monthStart, $lt: monthEndExclusive },
      },
      null,
      { companyId }
    ).select(
  ...
  const employees = profiles.map((profile) => {
    ...
    days.forEach((dayMeta) => {
      ...
      dayDetails[dayMeta.day] = {
        date: dayMeta.date,
        code,
        ...formatPunchSummary(record),
      };
      if (pendingLeave) {
