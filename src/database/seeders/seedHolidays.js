...
// Anchor to UTC midnight so the stored instant is identical no matter what
// local timezone the seeding machine/process is running in (a plain
// `new Date(y, m - 1, d)` uses the local timezone offset, which produced
// duplicate/off-by-one-day holiday records when seeded from hosts in
// different timezones).
