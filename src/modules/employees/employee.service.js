...
const updateEmployee = async (id, data, companyId, actorId, req, options = {}) => {
  ...
  if (data.officialEmail !== undefined) {
    const nextEmail = String(data.officialEmail).toLowerCase().trim();
    const currentUserEmail = profile.userId?.email?.toLowerCase();
    const currentProfileEmail = profile.officialEmail?.toLowerCase();

    if (nextEmail !== currentUserEmail || nextEmail !== currentProfileEmail) {
      const emailTaken = await User.findOne({
        email: nextEmail,
        _id: { $ne: profile.userId._id },
      });
      if (emailTaken) throw ApiError.conflict('Official email already registered');

      const profileEmailTaken = await employeeRepository.findOne(
        { companyId, officialEmail: nextEmail, _id: { $ne: id } },
        null,
        { companyId }
      );
      if (profileEmailTaken) throw ApiError.conflict('Official email already exists in this company');
    }
  }
