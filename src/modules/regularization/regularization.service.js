...
const { getDateOnly, formatDateOnly, getMonthYear, formatDateTimeIST, formatTimeIST } = require('../../utils/time');
...
const formatRequest = (doc) => {
  ...
  return {
    ...
    originalPunchInAt: doc.originalPunchInAt ? formatDateTimeIST(doc.originalPunchInAt) : null,
    originalPunchOutAt: doc.originalPunchOutAt ? formatDateTimeIST(doc.originalPunchOutAt) : null,
    originalStatus: doc.originalStatus || null,
    requestedPunchInAt: doc.requestedPunchInAt ? formatDateTimeIST(doc.requestedPunchInAt) : null,
    requestedPunchOutAt: doc.requestedPunchOutAt ? formatDateTimeIST(doc.requestedPunchOutAt) : null,
    ...
    approvedAt: status === 'approved' && doc.approvedAt ? formatDateTimeIST(doc.approvedAt) : null,
    ...
    cancelledAt: doc.cancelledAt ? formatDateTimeIST(doc.cancelledAt) : null,
    cancelledReason: doc.cancelledReason || null,
    createdAt: doc.createdAt ? formatDateTimeIST(doc.createdAt) : null,
    updatedAt: doc.updatedAt ? formatDateTimeIST(doc.updatedAt) : null,
  };
};
...
const getEligibility = async (userId, companyId, date) => {
  ...
  return {
    ...
    punchInAt: ctx.punchInAt ? formatDateTimeIST(ctx.punchInAt) : null,
    punchInDisplay: ctx.punchInAt ? formatTimeIST(ctx.punchInAt) : null,
    punchOutAt: ctx.punchOutAt ? formatDateTimeIST(ctx.punchOutAt) : null,
    punchOutDisplay: ctx.punchOutAt ? formatTimeIST(ctx.punchOutAt) : null,
    lateByMinutes: ctx.isWeeklyOff ? 0 : ctx.lateByMinutes,
    lateByDisplay: ctx.isWeeklyOff
      ? '0m'
      : (() => {
          const m = ctx.lateByMinutes || 0;
