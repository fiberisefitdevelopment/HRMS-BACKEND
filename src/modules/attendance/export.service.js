...
const exportReport = async (type, companyId, query, actorId, req, format = 'xlsx') => {
  ...
  const rows = report.data.map((r) => ({
    ...
    shift: r.shiftId?.name || r.shift?.name,
    punchIn: r.punchIn?.timestampDisplay || r.punchIn?.timestamp || null,
    punchOut: r.punchOut?.timestampDisplay || r.punchOut?.timestamp || null,
